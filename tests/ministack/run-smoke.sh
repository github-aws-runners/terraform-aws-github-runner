...
for command in aws curl docker openssl python3 rg terraform; do
...
attempts=30
while ! curl -fsS --max-time 2 -X PUT "http://127.0.0.1:${mock_port}/mockserver/status" >/dev/null 2>&1; do
  attempts=$((attempts - 1))
...
wait_for_mock_route() {
  ...
  attempts=60
  while ! curl -fsS --max-time 5 -X PUT "http://127.0.0.1:${mock_port}/mockserver/verify" \
    -H 'Content-Type: application/json' \
    ...
    if [ "$attempts" -le 0 ]; then
      ...
      curl -sS --max-time 5 -X PUT \
        "http://127.0.0.1:${mock_port}/mockserver/retrieve?type=REQUEST_RESPONSES&format=JSON" >&2 || true
      exit 1
    fi
    sleep 2
  done
