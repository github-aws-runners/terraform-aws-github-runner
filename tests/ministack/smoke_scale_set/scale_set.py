...
class ScaleSetSmoke:
    """Own the scale-set deployment and its controller/runner assertions."""

    example = "multi-runner-scale-set"
    group_name = "linux-scale-set"
    runner_name = "linux-scale-set"
    repository_name = "scale-set-controller"
    image_reference = "localhost:4566/scale-set-controller:smoke"
    runner_id = "321"
    routes = (
    ...
    def __init__(
        ...
        *,
        ...
        self.mock_host = self.environment.get("MINISTACK_GITHUB_MOCK_HOST", "127.0.0.1")
        try:
    ...
    def prepare(self) -> None:
        ...
        with self.step("Wait for MiniStack and MockServer"):
            ...
            fixture_path = self.script_dir / "initializerJson.json"
        ...
        with self.step("Build and publish the scale-set controller image"):
            ...
            self.aws("ecr", "create-repository", "--repository-name", self.repository_name,
    ...
    def _deploy_task_revision(self, arn: str, *, old_container: str | None = None) -> tuple[str, str]:
        ...
        def find_container():
            result = self.run([
                "docker", "ps", "-a", "--filter", f"label=com.amazonaws.ecs.task-definition-family={self.service_name}",
                "--filter", f"label=com.amazonaws.ecs.task-definition-version={revision}",
                "--filter", "name=scale-set-controller", "--format", "{{.ID}}",
            ], check=False)
            return next((line.strip() for line in result.stdout.splitlines() if line.strip() and line.strip() != old_container), None)
    ...
    def wait_controller_event(self, container: str, marker: str, required: str = "") -> list[str]:
        ...
        def contains_event() -> bool:
            nonlocal last_lines
            result = self.run(["docker", "logs", container], check=False)
    ...
    def execute(self) -> int:
        ...
        succeeded = False
        try:
            with self.step("Scale-set MiniStack integration smoke"):
                with self.step("Prepare controller image and deployment"):
                    self.prepare()
                with self.step("Verify ECS controller scale-up"):
                    definition = self.verify_ecs()
                    container = self.scale_up(definition)
                with self.step("Verify ECS controller scale-down"):
                    self.scale_down(definition, container)
                with self.step("Destroy deployment"):
                    if self.keep_deployment:
                        self.progress("Keeping the scale-set Terraform deployment for debugging")
                    else:
                        self.run([
                            str(self.source_root / "tests/ministack/run-example.sh"), "destroy", self.example,
                            str(self.tfvars_path),
                        ], stream=True)
                        self.terraform_applied = False
                    self.mark("cleanup")
                if not self.keep_deployment:
                    with self.step("Verify controller session cleanup"):
                        self.wait_for(
            ...
            self.progress(
                f"Scale-set MiniStack ECS/MockServer smoke test passed for {self.provider.display_name}."
            )
            succeeded = True
            return 0
            ...
            self._write_checklist("passed" if succeeded else "failed")
