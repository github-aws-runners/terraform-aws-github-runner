...
An omitted `org` uses `pool_runner_owner`. Multi-org pools must have a valid organization login in one of those fields. Use the organization's login as returned by GitHub, rather than its display name. Without the flag, `org` is ignored and the existing default owner is used.
...
- EC2 already persists the organization in `ghr:Owner` alongside `ghr:Type = Org`. Scale-down, deregistration, and orphan checks use that ownership metadata; no additional tag is required. Other compute providers use the equivalent owner/type fields in their provider contract.
