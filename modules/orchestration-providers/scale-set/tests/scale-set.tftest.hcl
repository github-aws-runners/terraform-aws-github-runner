...
run "rejects_controller_group_policy_above_inline_quota" {
  ...
  plan_options {
    target = [terraform_data.validate_group_compute_policy["ec2"]]
  }

  override_data {
    target = data.aws_iam_policy_document.task_compute["ec2"]
    values = {
      json = <<-JSON
  ...
  expect_failures = [terraform_data.validate_group_compute_policy["ec2"]]
}
