...
| <a name="input_storage_provider"></a> [storage\_provider](#input\_storage\_provider) | Opaque runner-configuration storage capability used by the job-retry Lambda. | <pre>object({<br/>    type                  = string<br/>    environment_variables = map(string)<br/>    iam_policy_json       = optional(string, null)<br/>  })</pre> | <pre>{<br/>  "environment_variables": {},<br/>  "iam_policy_json": null,<br/>  "type": "aws_ssm"<br/>}</pre> | no |
