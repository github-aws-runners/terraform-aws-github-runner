...
| <a name="input_storage_provider"></a> [storage\_provider](#input\_storage\_provider) | Resolved storage-provider configuration and capability used by the job-retry Lambda. | <pre>object({<br/>    aws = object({<br/>      ssm = optional(object({<br/>        kms_key_id = optional(string, null)<br/>      }), null)<br/>    })<br/>    environment_variables = optional(map(string), {})<br/>    iam_policy_json       = optional(string, null)<br/>  })</pre> | n/a | yes |
