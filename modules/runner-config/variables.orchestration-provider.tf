...
variable "orchestration" {
  ...
  validation {
    condition = var.orchestration.webhook == null ? true : (
      var.orchestration.webhook.lambda.scale_up.event_source_mapping.batch_size >= 1 &&
      var.orchestration.webhook.lambda.scale_up.event_source_mapping.batch_size <= 1000 &&
      var.orchestration.webhook.lambda.scale_up.event_source_mapping.maximum_batching_window_in_seconds >= 0 &&
      var.orchestration.webhook.lambda.scale_up.event_source_mapping.maximum_batching_window_in_seconds <= 300
    )
    error_message = "orchestration.webhook.lambda.scale_up.event_source_mapping batch size must be between 1 and 1000 and its batching window between 0 and 300 seconds."
  }

  validation {
    condition = var.orchestration.webhook == null ? true : !(
      var.orchestration.webhook.lambda.scale.artifact.zip != null &&
      var.orchestration.webhook.lambda.scale.artifact.s3 != null
    )
    error_message = "orchestration.webhook.lambda.scale.artifact must select at most one of zip or s3."
  }
