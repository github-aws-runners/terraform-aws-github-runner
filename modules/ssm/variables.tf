variable "github_app" {
  ...
  validation {
    ...
    error_message = "You must set `webhook_secret` or `webhook_secret_ssm` in the `github_app` variable."
  }
}

variable "runner_registration_level" {
  description = "The level at which runners are registered: \"repo\", \"org\", or \"enterprise\". When \"enterprise\", the GitHub App id/key are not required (a PAT is used instead)."
  type        = string
  default     = null
}
