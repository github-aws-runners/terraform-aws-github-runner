...
module "ec2_runner_role" {
  ...
  enable_ssm_on_runners         = local.ec2.enable_ssm_on_runners
...
locals {
  ...
  create_runner_role   = local.provider_type == "ec2" && var.runner_iam.role == null

  runner_role = {
    arn  = local.create_runner_role ? one(aws_iam_role.runner[*].arn) : var.runner_iam.role.arn
    name = local.create_runner_role ? one(aws_iam_role.runner[*].name) : basename(var.runner_iam.role.arn)
  }

  runner_managed_policy_arns = merge(
    {
      for policy_name, policy_arn in var.runner_iam.managed_policy_arns :
      "user-${policy_name}" => policy_arn
    },
    var.tracing_config.mode != null ? {
...
resource "aws_iam_role" "runner" {
  ...
  lifecycle {
    precondition {
      condition     = local.ec2.instance_profile == null || var.runner_iam.role != null
      error_message = "runner_iam.role must be set when compute_provider.ec2.instance_profile selects an external instance profile."
    }
  }
}
