...
The resolved provider-neutral `runner.iam.role` is passed to Lambda as the MicroVM execution role. The provider creates `/github-self-hosted-runners/<prefix>/microvm` with the common observability lifecycle. When the runner role is supplied externally, its trust and stream-write permissions remain caller-owned.
