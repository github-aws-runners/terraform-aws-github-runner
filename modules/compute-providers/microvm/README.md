...
| <a name="input_runner"></a> [runner](#input\_runner) | Provider-neutral runner settings consumed by MicroVM.<br/><br/>- `boot_time_in_minutes`: Expected boot and registration duration used by scale-down and pool.<br/>- `name_prefix`: Prefix added to registered runner names.<br/>- `iam.role.arn`: Resolved common runner role ARN used as the default MicroVM execution role.<br/>- `iam.role.name`: Resolved common runner role name.<br/>- `iam.role.managed`: Whether runner-stack manages the resolved runner role.<br/>- `iam.path`: IAM path associated with the resolved runner role. Null derives the path from `prefix`. | <pre>object({<br/>    boot_time_in_minutes = optional(number, 5)<br/>    name_prefix          = optional(string, "")<br/>    iam = object({<br/>      role = object({<br/>        arn     = string<br/>        name    = string<br/>        managed = optional(bool, true)<br/>      })<br/>      path = optional(string, null)<br/>    })<br/>  })</pre> | n/a | yes |
