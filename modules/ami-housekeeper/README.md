...
|------|-------------|------|---------|:--------:|
| <a name="input_ami_cleanup_config"></a> [ami\_cleanup\_config](#input\_ami\_cleanup\_config) | Configuration for AMI cleanup.<br><br>    `maxItems` - The maximum numer of AMI's tha will be queried for cleanup. Default no maximum.<br>    `minimumDaysOld` - Minimum number of days old an AMI must be to be considered for cleanup. Default 30.<br>    `amiFilters` - Filters to use when searching for AMIs to cleanup. Default filter for images owned by the account and that are available.<br>    `launchTemplateNames` - Launch template names to use when searching for AMIs to cleanup. Default no launch templates.<br>    `ssmParameterNames` - SSM parameter names to use when searching for AMIs to cleanup. This parameter should be set when using SSM to configure the AMI to use. Default no SSM parameters.<br>    `dryRun` - If true, no AMIs will be deregistered. Default false. | <pre>object({<br>    maxItems       = optional(number)<br>    minimumDaysOld = optional(number)<br>    amiFilters = optional(list(object({<br>      Name   = string<br>      Values = list(string)<br>    })))<br>    launchTemplateNames = optional(list(string))<br>    ssmParameterNames   = optional(list(string))<br>    dryRun              = optional(bool)<br>  })</pre> | `{}` | no |
...
| <a name="input_lambda_schedule_expression"></a> [lambda\_schedule\_expression](#input\_lambda\_schedule\_expression) | Scheduler expression for action runner binary syncer. | `string` | `"cron(11 7 * * ? *)"` | no |
