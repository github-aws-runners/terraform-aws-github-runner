...
| <a name="input_instance_minimum_running_time_in_minutes"></a> [instance\_minimum\_running\_time\_in\_minutes](#instance\_input\_minimum\_running\_time\_in\_minutes) | The time an ec2 action runner should be running at minimum before terminated if non busy. If not set the default is calculated based on the OS. | `number` | `null` | no |
