...
resource "aws_sqs_queue" "queued_builds" {
  for_each                   = local.legacy_multi_runner_config
...
resource "aws_sqs_queue" "queued_builds_dlq" {
  for_each = { for config, values in local.legacy_multi_runner_config : config => values if values.redrive_build_queue.enabled }
