  ...
  description = <<EOT
    multi_runner_config = {
      runner_config: {
        ...
        enable_organization_runners: "DEPRECATED: Use `runner_registration_level` instead. Register runners to organization, instead of repo level. If set to `true`, it takes priority over `runner_registration_level` for backwards compatibility."
        runner_registration_level: "The level at which runners are registered in GitHub. Valid values: `repo`, `org`, `enterprise`. Defaults to `repo` when not set."
