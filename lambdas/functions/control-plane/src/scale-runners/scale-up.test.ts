...
const EXPECTED_ENTERPRISE_RUNNER_PARAMS: RunnerInputParameters = {
  environment: 'unit-test-environment',
...
function setDefaults() {
  ...
  process.env.PARAMETER_ENTERPRISE_PAT_NAME = 'github-enterprise-pat';
  process.env.GITHUB_APP_KEY_BASE64 = 'TEST_CERTIFICATE_DATA';
  process.env.GITHUB_APP_ID = '1337';
  process.env.GITHUB_APP_CLIENT_ID = 'TEST_CLIENT_ID';
  process.env.GITHUB_APP_CLIENT_SECRET = 'TEST_CLIENT_SECRET';
  process.env.GITHUB_ENTERPRISE_PAT = 'TEST_PAT';
...
describe('scaleUp with GHES', () => {
  ...
  describe('on enterprise level', () => {
    beforeEach(() => {
      ...
      expectedRunnerParams = { ...EXPECTED_ENTERPRISE_RUNNER_PARAMS };
      mockSSMClient.reset();
    });
    ...
    it('create start runner config for non-ephemeral runners ', async () => {
      ...
      expect(mockSSMClient).toHaveReceivedNthSpecificCommandWith(1, PutParameterCommand, {
        Name: '/github-action-runners/default/runners/config/i-12345',
        Value:
          '--url https://github.enterprise.something/enterprise/ --token 1234abcd ' +
...
function defaultOctokitMockImpl() {
  ...
  routeHandlers.set('POST /enterprises/{enterprise}/actions/runners/generate-jitconfig', () =>
    Promise.resolve({
      data: {
  ...
  routeHandlers.set('POST /enterprises/{enterprise}/actions/runners/registration-token', () =>
    Promise.resolve({
      data: {
        token: '1234abcd',
      },
...
function defaultSSMGetParameterMockImpl() {
  mockSSMgetParameter.mockImplementation(async (name: string) => {
    ...
    } else if (name === `${process.env.PARAMETER_ENTERPRISE_PAT_NAME}`) {
      return `${process.env.GITHUB_ENTERPRISE_PAT}`;
    } else {
