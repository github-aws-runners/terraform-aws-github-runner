...
export const handler = async (_event: any, context: Context): Promise<void> => {
  try {
    ...
    const result = await amiManager.updateLaunchTemplate(
      config.launchTemplateName,
      latestAmiId,
      config.dryRun
    );

    if (result.success) {
      metrics.addMetric('AMIUpdateSuccess', 1);
      logger.info('AMI update completed successfully', { result });
    } else {
      metrics.addMetric('AMIUpdateFailure', 1);
      logger.error('AMI update failed', { result });
      throw new Error(result.message);
    }
  } catch (error) {
    metrics.addMetric('AMIUpdateError', 1);
