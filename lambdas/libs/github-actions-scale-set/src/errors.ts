...
export class ScaleSetHttpError extends Error {
  ...
  constructor(details: ScaleSetHttpErrorDetails) {
    ...
    super(`request ${details.method} ${details.url} failed (${metadata}): ${exceptionPrefix}${responseMessage}`, {
      cause: details.cause,
    });
...
export class ScaleSetRequestError extends Error {
  ...
  constructor(method: string, url: string, cause: unknown, attempts = 1) {
    super(`request ${method} ${url} failed before receiving a response after ${attempts} attempt(s)`, { cause });
