...
export function validateRunnerId(runnerId: string): void {
  if (!RUNNER_ID_PATTERN.test(runnerId)) throw new Error('runnerId is invalid');
}

export function canonicalSsmTokenPath(tokenPath: string): string {
  if (tokenPath.includes('//')) throw new Error('aws_ssm tokenPath is invalid');
...
export function delay(ms: number, signal: AbortSignal): Promise<void> {
  if (signal.aborted) return Promise.reject(new Error('runner configuration consumption was cancelled'));
...
export async function withCallDeadline<T>(
  ...
  if (parentSignal.aborted) throw new Error('runner configuration consumption was cancelled');
  const remaining = deadlineMs - Date.now();
  if (remaining <= 0) throw new RunnerConfigCallDeadlineError();
...
export function throwIfCancelled(signal: AbortSignal): void {
  if (signal.aborted) throw new Error('runner configuration consumption was cancelled');
}
