...
import type { AwsDynamoDbRunnerConfigApi } from './aws/dynamodb/runner-config-consumer';
...
describe('runner config storage context', () => {
  ...
  it('parses and freezes an exact DynamoDB environment map', () => {
    const context = parseRunnerConfigStorageContext(dynamoDbContext);

    expect(context).toEqual(dynamoDbContext);
    expect(Object.isFrozen(context)).toBe(true);
  });
  ...
  it.each([
    ...
    { ...ssmContext, SSM_TOKEN_PATH: '/runner/../tokens' },
    { RUNNER_CONFIG_STORAGE_PROVIDER: 'aws_dynamodb', RUNNER_CONFIG_DYNAMODB_RUNNER_STATE_TABLE_NAME: 'ab' },
    {
      RUNNER_CONFIG_STORAGE_PROVIDER: 'aws_dynamodb',
      RUNNER_CONFIG_DYNAMODB_RUNNER_STATE_TABLE_NAME: 'runner state',
    },
    { ...dynamoDbContext, SSM_TOKEN_PATH: '/runner/tokens' },
  ...
  it.each([
    ...
    [
      {
        ...dynamoDbContext,
        RUNNER_CONFIG_DYNAMODB_CONFIG_TABLE_NAME: 'durable-config',
        RUNNER_CONFIG_DYNAMODB_ENTRY_ID: 'linux',
        RUNNER_CONFIG_DYNAMODB_TTL_SECONDS: '3600',
        RUNNER_CONFIG_STORAGE_VERSION: 'version-hash',
      },
      dynamoDbContext,
    ],
  ...
  it('round-trips each producer environment through payload context and hook environment export', () => {
    for (const producerEnvironment of [ssmContext, dynamoDbContext]) {
      ...
      const hookEnvironment: Record<string, string | undefined> = {
        RUNNER_CONFIG_DYNAMODB_RUNNER_STATE_TABLE_NAME: 'stale-table',
        SSM_TOKEN_PATH: '/stale/path',
        UNRELATED: 'preserved',
      };

      expect(exportRunnerConfigStorageEnvironment(payloadContext, hookEnvironment)).toEqual(payloadContext);
      expect(loadRunnerConfigStorageContextFromEnvironment(hookEnvironment)).toEqual(payloadContext);
      expect(hookEnvironment.UNRELATED).toBe('preserved');
      if (payloadContext.RUNNER_CONFIG_STORAGE_PROVIDER === 'aws_ssm') {
        expect(hookEnvironment.RUNNER_CONFIG_DYNAMODB_RUNNER_STATE_TABLE_NAME).toBeUndefined();
      } else {
        expect(hookEnvironment.SSM_TOKEN_PATH).toBeUndefined();
      }
    }
  });
  ...
  it('does not mutate a target when context validation fails', () => {
    ...
    expect(() =>
      exportRunnerConfigStorageEnvironment({ ...dynamoDbContext, AWS_SECRET_ACCESS_KEY: 'forbidden' } as never, target),
    ).toThrow();
...
describe('runner config consumer environment factory', () => {
  ...
  it('creates an injected DynamoDB consumer using microvmId as the access scope', async () => {
    const api: AwsDynamoDbRunnerConfigApi = {
      deleteItem: vi.fn().mockResolvedValue('encoded-jit'),
    };
    const consumer = createRunnerConfigConsumerFromEnvironment(dynamoDbContext, {
      awsDynamoDbApi: api,
      callTimeoutMs: 100,
      configTimeoutMs: 100,
      pollIntervalMs: 1,
    ...
    expect(api.deleteItem).toHaveBeenCalledWith('runner-state', 'microvm-123', expect.any(AbortSignal));
  });
